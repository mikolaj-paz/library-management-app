...
class ReaderTest {
  ...
  void should_not_be_blocked_when_reader_is_active() {
    var reader = new ReaderFactoryImpl().reconstitute(ReaderId.create(), ReaderStatus.ACTIVE, 0);
  ...
  void should_be_blocked_when_reader_status_is_blocked() {
    var reader = new ReaderFactoryImpl().reconstitute(ReaderId.create(), ReaderStatus.BLOCKED, 0);
